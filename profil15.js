  ...
  api('/api/aktuallar?tg_id=' + myId()).then(function(d){
    ...
    box.innerHTML =
      ...
      list.map(function(x){
        ...
        return '<button class="hl" data-u="' + u + '" data-v="' + (x.is_video ? '1' : '0') + '" data-id="' + (x.id || '') + '" data-t="' + esc(x.title || '') + '">' +
    ...
    box.querySelectorAll('.hl[data-u]').forEach(function(b){
      ...
      b.addEventListener('touchstart', function(){
        lt = setTimeout(function(){
          if(confirm('"' + b.dataset.t + '" aktualini ochirasizmi?')){
            apiPost('/api/aktual-ochirish', { id: b.dataset.id }).then(function(d){
              if(d && d.ok){ toast('Ochirildi'); loadHl(); }
              else toast('Xato');
...
function editProfileU(){
  ...
  gb.addEventListener('click', function(){
    ...
    const nm = bg.querySelector('#euName').value.trim();
    gb.disabled = true;
    gb.textContent = 'Saqlanmoqda...';

    Promise.all([
      apiPost('/api/bio', { bio: bio }),
      nm ? apiPost('/api/profil-saqlash', { name: nm }) : Promise.resolve({ ok: true })
    ]).then(function(res){
      var r = res[0];
      if(r.ok){
        haptic('medium');
  ...
  pickFiles(true, function(items){
    ...
    bg.innerHTML = '<div class="sheet" id="upS" style="max-height:92vh;overflow-y:auto;">' +
      ...
      ((typeof IS_ADMIN !== 'undefined' && IS_ADMIN)
        ...
        : '') +
      '<div class="geo-pick" id="upCol"><span class="ic">&#129309;</span>' +
        '<span class="tx"><b>Video ulash</b>' +
        '<span id="upColN">Hammualif tanlang</span></span></div>' +
      '<div style="margin:12px 0;padding:13px;border-radius:13px;' +
        'background:rgba(232,196,104,.07);border:1px solid rgba(232,196,104,.22)">' +
        '<label style="display:flex;align-items:center;gap:10px;cursor:pointer">' +
        '<input type="checkbox" id="upBron" style="width:19px;height:19px;flex:none">' +
        '<b style="font-size:14px">Bron qilish imkonini yoqish</b></label>' +
        '<div id="upBronBox" style="display:none;margin-top:12px">' +
        '<div style="font-size:11.5px;color:#9A9A90;margin-bottom:6px">' +
        'Bron summasi (narxdan foiz)</div>' +
        '<select id="upBronFoiz" style="width:100%;padding:11px;border-radius:10px;' +
        'background:rgba(0,0,0,.35);border:1px solid rgba(255,255,255,.12);' +
        'color:#fff;font-family:inherit;font-size:14px">' +
        '<option value="10">10%</option><option value="20">20%</option>' +
        '<option value="30">30%</option><option value="40">40%</option>' +
        '<option value="50">50%</option></select>' +
        '<div style="font-size:11.5px;color:#9A9A90;margin:10px 0 6px">' +
        'Karta raqami (bron puli shu kartaga tushadi)</div>' +
        '<input id="upKarta" inputmode="numeric" placeholder="8600 1234 5678 9012" ' +
        'style="width:100%;padding:11px;border-radius:10px;background:rgba(0,0,0,.35);' +
        'border:1px solid rgba(255,255,255,.12);color:#fff;font-family:inherit;font-size:14px">' +
        '<div style="font-size:11.5px;color:#9A9A90;margin:10px 0 6px">' +
        'Karta egasi</div>' +
        '<input id="upKartaEgasi" placeholder="ISMOILOV AZIZ" ' +
        'style="width:100%;padding:11px;border-radius:10px;background:rgba(0,0,0,.35);' +
        'border:1px solid rgba(255,255,255,.12);color:#fff;font-family:inherit;' +
    ...
    let tagged = [];
    let hamm = [];
    const bronCb = bg.querySelector('#upBron');
    if(bronCb) bronCb.onchange = function(){
      const b = bg.querySelector('#upBronBox');
      if(b) b.style.display = bronCb.checked ? '' : 'none';
    };

    const col_ = bg.querySelector('#upCol');
    if(col_) col_.addEventListener('click', function(){
      pickPeople(hamm, function(sel){
        hamm = sel;
        col_.classList.toggle('on', sel.length > 0);
        bg.querySelector('#upColN').textContent = sel.length
          ? sel.length + ' ta hammualif' : 'Hammualif tanlang';
      });
    });
    ...
    gb.addEventListener('click', function(){
      ...
      _pre.then(function(){
      const _bron = bg.querySelector('#upBron');
      const _bOn = _bron && _bron.checked ? 1 : 0;
      const _bFoiz = _bOn ? parseInt((bg.querySelector('#upBronFoiz')||{}).value || 0) : 0;
      const _bKarta = _bOn ? ((bg.querySelector('#upKarta')||{}).value || '').trim() : '';
      const _bEgasi = _bOn ? ((bg.querySelector('#upKartaEgasi')||{}).value || '').trim() : '';
      if(_bOn && (!_bKarta || !_bEgasi)){
        alert('Bron uchun karta raqami va egasini toldiring');
        return Promise.reject(new Error('karta_yoq'));
      }
      return apiPost('/api/post-qoshish', {
        bron_on: _bOn, bron_foiz: _bFoiz,
        karta: _bKarta, karta_egasi: _bEgasi,
        ...
        if(d.ok && tagged.length){
          setTimeout(function(){
            api('/api/mening-postlarim?tg_id=' + myId()).then(function(r){
              ...
              if(last && hamm.length){
                var _u = 0;
                try { _u = parseInt(localStorage.getItem('ni_uid')) || 0; } catch(e){}
                hamm.forEach(function(h){
                  apiPost('/api/hammualif', {
                    post_id: last.id,
                    kim: (h && h.tg_id) ? h.tg_id : h,
                    tg_id: _u
                  }).catch(function(){});
                });
              }
              if(last && tagged.length) apiPost('/api/belgilash', {
    ...
    v.innerHTML =
      ...
      '<div class="pf-info">' +
        ...
        '<div class="pf-stats">' +
          ...
          (p.created && typeof timeAgo === 'function'
            ? '<span>\u{1F551} ' + timeAgo(p.created) + '</span>' : '') +
        '</div></div>' +

      ((PF && PF.username && p.username &&
        String(PF.username).toLowerCase() === String(p.username).toLowerCase())
        ? '<div class="pf-acts">' +
      ...
      /* ---- BRON ---- */
      ((p.bron_on && !(PF && PF.username && p.username &&
        String(PF.username).toLowerCase() === String(p.username).toLowerCase()))
        ? '<div style="margin:12px;padding:15px;border-radius:15px;' +
            'background:rgba(232,196,104,.08);border:1px solid rgba(232,196,104,.26)">' +
          (p.bron_holat
            ? '<div style="text-align:center;padding:8px 0">' +
              '<div style="font-size:17px;font-weight:800;color:#5BBF6A">' +
              '\u2705 Bron qilindi</div>' +
              '<div style="font-size:12px;color:#9A9A90;margin-top:4px">' +
              'Bu e\u2019lon band</div></div>'
            : '<div style="font-size:14px;font-weight:800;color:#F0C868;' +
              'margin-bottom:9px">\u{1F511} Bron qilish mumkin</div>' +
              '<div style="font-size:12.5px;color:#C9C9C0;margin-bottom:11px">' +
              'Bron summasi: narxning <b>' + (p.bron_foiz || 0) + '%</b></div>' +
              (p.karta
                ? '<div style="padding:11px;border-radius:11px;' +
                  'background:rgba(0,0,0,.3);margin-bottom:11px">' +
                  '<div style="font-size:10.5px;color:#7A7A72">Karta raqami</div>' +
                  '<div style="font-size:15px;font-weight:700;letter-spacing:.6px;' +
                  'margin:3px 0">' + E(p.karta) + '</div>' +
                  '<div style="font-size:11.5px;color:#9A9A90">' +
                  E(p.karta_egasi || '') + '</div>' +
                  '<button id="pfKartaNusxa" style="width:100%;margin-top:9px;' +
                  'padding:8px;border-radius:9px;border:1px solid rgba(232,196,104,.3);' +
                  'background:rgba(232,196,104,.1);color:#F0C868;font-family:inherit;' +
                  'font-size:12px;font-weight:700;cursor:pointer">Nusxa olish</button>' +
                  '</div>'
                : '') +
              '<div style="padding:11px;border-radius:11px;' +
              'background:rgba(232,67,61,.09);border:1px solid rgba(232,67,61,.24);' +
              'margin-bottom:11px;font-size:11.5px;color:#F0A8A5;line-height:1.55">' +
              '\u26A0\uFE0F <b>Diqqat!</b> Maklerni surishtirib, ishonch hosil qilgandan ' +
              'keyin pul o\u2019tkazing. UYgram o\u2019tkazgan pulingizga javob bermaydi.' +
              '</div>' +
              '<button id="pfBron" style="width:100%;padding:13px;border-radius:12px;' +
              'border:none;background:linear-gradient(135deg,#F0C868,#D4AF37);' +
              'color:#12100A;font-family:inherit;font-size:14px;font-weight:800;' +
              'cursor:pointer">Bron qilish uchun so\u2019rov yuborish</button>') +
          '</div>'
        : '');

    const kn = v.querySelector('#pfKartaNusxa');
    if(kn) kn.onclick = function(){
      try {
        navigator.clipboard.writeText(String(p.karta || '').replace(/\s/g,''));
        kn.textContent = 'Nusxa olindi';
        setTimeout(function(){ kn.textContent = 'Nusxa olish'; }, 1600);
      } catch(e){ if(typeof toast === 'function') toast('Nusxa olinmadi'); }
    };

    const bt = v.querySelector('#pfBron');
    if(bt) bt.onclick = function(){
      var tel = prompt('Telefon raqamingiz (makler bogllanadi):', '');
      if(tel === null) return;
      bt.disabled = true;
      bt.textContent = 'Yuborilmoqda...';
      apiPost('/api/bron', {
        post_id: p.id, tel: tel,
        summa: String(p.price || '') + ' dan ' + (p.bron_foiz || 0) + '%'
      }).then(function(d){
        if(d && d.ok){
          bt.textContent = 'Sorov yuborildi';
          if(typeof toast === 'function')
            toast('Makler tasdiqlashini kuting. Chekni maklerga yuboring.');
        } else {
          bt.disabled = false;
          bt.textContent = 'Bron qilish uchun sorov yuborish';
          var e = (d && d.error) || '';
          if(typeof toast === 'function')
            toast(e === 'band' ? 'Bu elon allaqachon bron qilingan'
                : e === 'yoqilmagan' ? 'Bron yoqilmagan' : 'Yuborilmadi');
        }
      }).catch(function(){
        bt.disabled = false;
        bt.textContent = 'Bron qilish uchun sorov yuborish';
        if(typeof toast === 'function') toast('Server xatosi');
      });
    };
...
function postMenu(p){
  ...
  bg.innerHTML = '<div class="sheet" id="pmS"><div class="sheet-bar"></div>' +
    ...
    '<button class="sheet-item" data-a="hammualif">&#129309;&nbsp;&nbsp;Hammualif qoshish</button>' +
...
(function(){
  ...
  function aBody(x){
    ...
    try {
      if(!b.tg_id){
        var tu = window.Telegram && window.Telegram.WebApp
          && window.Telegram.WebApp.initDataUnsafe
          && window.Telegram.WebApp.initDataUnsafe.user;
        if(tu && tu.id) b.tg_id = tu.id;
      }
      if(!b.tg_id && typeof myId === 'function'){
        var m = myId();
        if(m) b.tg_id = m;
      }
    } catch(e){}
  ...
  window.showAiChat = function(){
    ...
    function yubor(){
      ...
      fetch(AA + '/api/ai', {
        ...
        kut.textContent = 'XATO: ' + (er && er.message ? er.message : String(er));
      });
    }
...
window.showAiChat = function(){
  ...
  function yubor(){
    ...
    apiPost('/api/ai', { matn: t }).then(function(d){
      kut.remove();
